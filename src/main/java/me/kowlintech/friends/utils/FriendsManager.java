...
public class FriendsManager {
    ...
    public static void addFriendRequest(String triggeredRequest, String receivedRequest) throws SQLException {
        ...
        Friends.connection.createStatement().execute("INSERT INTO requests (thatrequested, other, whenrequested) VALUES ('" + triggeredRequest + "', '" + receivedRequest + "', '" + dateString + "')");
    }

    public static void removeFriendRequest(String playera, String playerb) throws SQLException {
        Friends.connection.createStatement().execute("DELETE FROM requests WHERE thatrequested='" + playera + "' AND other='" + playerb + "'");
    }
    ...
    public static ArrayList<String> incomingRequests(String player) throws SQLException {
        ...
        PreparedStatement st = Friends.connection.prepareStatement("SELECT * FROM requests WHERE other='" + player + "'");
        ResultSet rs = st.executeQuery();
        while (rs.next()) {
    ...
    public static ArrayList<String> outgoingRequests(String player) throws SQLException {
        ...
        PreparedStatement st = Friends.connection.prepareStatement("SELECT * FROM requests WHERE thatrequested='" + player + "'");
        ResultSet rs = st.executeQuery();
        while (rs.next()) {
